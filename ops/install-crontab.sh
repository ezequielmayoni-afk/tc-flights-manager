...
#
# Qué hace en el VPS:
#   1. copia ops/hub-cron.sh a /opt/hub-cron/ (ejecutable). Va fuera de /opt/hub
#      porque el deploy hace rsync --delete ahí y borraría el script.
#   2. si no existe /opt/hub-cron/.env.cron, lo crea tomando CRON_SECRET de
...
$SSH "mkdir -p /opt/hub-cron && chmod 700 /opt/hub-cron"
scp -q -i "$SSH_KEY" "$HERE/hub-cron.sh" "$VPS_USER@$VPS_HOST:/opt/hub-cron/hub-cron.sh"
$SSH "chmod 750 /opt/hub-cron/hub-cron.sh"

echo "→ preparando /opt/hub-cron/.env.cron"
$SSH 'set -e
if [ ! -f /opt/hub-cron/.env.cron ]; then
  ...
  umask 077
  printf "CRON_SECRET=%s\n" "$secret" > /opt/hub-cron/.env.cron
  chmod 600 /opt/hub-cron/.env.cron
  echo "   creado (600)"
else
