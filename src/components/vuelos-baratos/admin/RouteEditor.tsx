...
const EMPTY: RouteFormValues = { origin_tc_code: 'BUE', origin_name: 'Buenos Aires', stay_nights: [7, 10, 14], weekdays: [2, 5], probes_per_month: 8, months_ahead: 12, active: false }
...
export function RouteEditor({ mode, destinationCode, destinationName, initial, onClose }: { mode: 'create' | 'edit'; destinationCode: string; destinationName: string; initial?: RouteFormValues; onClose: () => void }) {
  ...
  async function save() {
    ...
    try {
      ...
      const payload = { origin_tc_code: v.origin_tc_code.trim().toUpperCase(), origin_name: v.origin_name.trim(), stay_nights, weekdays: v.weekdays, probes_per_month: v.probes_per_month, months_ahead: v.months_ahead, active: v.active }
  ...
  const porNoche = v.probes_per_month * v.months_ahead
    ...
    <Dialog open onOpenChange={open => { if (!open) onClose() }}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>Activa, esta ruta suma ~{porNoche} búsquedas por noche en siviajo.com (sondas/mes × meses).</DialogDescription>
        </DialogHeader>
