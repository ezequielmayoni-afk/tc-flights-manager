...
import type { TrendAlert, TrendDestination, TrendRunOptions, TrendRunResult } from './types'
...
export async function runTendencias(options: TrendRunOptions): Promise<TrendRunResult> {
  ...
  try {
    const { results, sourcesCollected, serpApiCalls } = await collectAll(options)
    ...
    if (!dryRun && runId) await persist(db, runId, destinations, alerts)
    ...
    return { runId, weekLabel, status: 'completed', destinations, alerts, sourcesCollected, catalogSnapshotCount: withPackages, serpApiCalls, durationMs }
  } catch (err) {
    const durationMs = Date.now() - started
    ...
    return { runId, weekLabel, status: 'failed', destinations: [], alerts: [], sourcesCollected: {}, catalogSnapshotCount: 0, serpApiCalls: options.serpapi.callsMade, durationMs, error: message }
  }
}
...
async function persist(db: Db, runId: string, destinations: TrendDestination[], alerts: TrendAlert[]): Promise<void> {
  const rows = destinations.map(d => ({
    ...
    rank: d.rank,
    signal_google_trends: d.signals.googleTrends,
    signal_autocomplete: d.signals.autocomplete,
    signal_search_console: d.signals.searchConsole,
    signal_amadeus_price: d.signals.amadeusPrice,
    signal_news_events: d.signals.newsEvents,
    signal_reddit: d.signals.reddit,
